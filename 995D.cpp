...
void solve(){
    ...
    for (int i = 0; i < n; i++) {
        long long lower = ts - nums[i] - y;
        long long upper = ts - nums[i] - x;
        auto it_low = lower_bound(nums.begin() + i + 1, nums.end(), lower);
        auto it_up = upper_bound(nums.begin() + i + 1, nums.end(), upper);

        ans += distance(it_low, it_up);
    }
    // for(int i=0;i<n; i++){
    // 	for(int j=i+1; j<n; j++){
    // 		if(ts <= nums[i]+nums[j]+y && ts >= nums[i]+nums[j]+x) ans++;
    // 	}
    // }
    // unordered_map<int, int> freq;

    // for (int i = 0; i < n; i++) {
    //     int lower = ts - nums[i] - y;
    //     int upper = ts - nums[i] - x;
